...
def search_genbank(gene, species, email):
    ...
    handle = Entrez.esearch(db="protein", term=query, idtype="acc", retmode="text")
...
def retrieve_protein_sequence(seq_ID, email):
    Entrez.email = email
...
def create_fasta_file(gene, species_list, tax_level, email):
    ...
    for species in species_list:
        ...
        if ID_result:
            sequence = retrieve_protein_sequence(ID_result, email)
